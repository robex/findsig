...
uint8_t *pat2bytes(char *pat_str, int *pat_len)
{
	int pat_str_len = strlen(pat_str);
	uint8_t *pat_buf = malloc(pat_str_len);
	...
	pat_buf = realloc(pat_buf, nbytes);
	*pat_len = nbytes;
	return pat_buf;
}

char *pat2mask(char *pat_str, int *mask_len)
{
	int pat_str_len = strlen(pat_str);
	char *mask = malloc(pat_str_len);
...
uint8_t *read_file(FILE *f, size_t *file_sz)
{
...
long find_sig(uint8_t *file_buf, size_t file_sz, uint8_t *pat_buf, int pat_len, char *mask)
{
	if (pat_len > file_sz) {
		return -1;
	}
